...
{
    [Authorize]
    public class A42Model : PageModel, IDisposable
    {
        private readonly AppServices _s;
        public A42Model(AppServices s)
        {
            _s = s;
        }
        ...
        public CombinationStatisticsResult? Statistics { get; set; }
        public bool HasTime { get; set; }

        public class CombinationStatisticsResult
        {
            public string CombinationString { get; set; } = "";
            public List<int> CombinationNumbers { get; set; } = new();
        ...
        {
            string cacheKey = $"A42_{Game}_{DrawCount}_{Direction}";
            if (!string.IsNullOrEmpty(Game) && !string.IsNullOrEmpty(Combination))
            {
        ...
        private async Task LoadStatistics()
        {
            ...
            // Разделяем комбинацию, введённую пользователем, на отдельные числа по пробелам
            if (!int.TryParse(Combination.Split(' ', StringSplitOptions.RemoveEmptyEntries).FirstOrDefault(), out _))
            {
                return;
            }

            var selectedCombination = Combination.Split(' ', StringSplitOptions.RemoveEmptyEntries).Select(int.Parse).ToList();
            ...
            foreach (var rowObj in draws)
            {
                ...
                if (selectedCombination.All(drawBalls.Contains))
                {
                    occurrences++;
                    ...
                    if (dateValue != null)
                    {
                        ...
                        // Проверяем, новее ли текущая дата
            ...
            Statistics = new CombinationStatisticsResult
            {
                CombinationString = Combination,
                CombinationNumbers = selectedCombination,
        ...
        public async Task<IActionResult> OnPostDownloadCsv()
        {
            if (string.IsNullOrEmpty(Game) || string.IsNullOrEmpty(Combination))
            {
                return BadRequest("Игра и комбинация обязательны.");
            }

            return await _s.Csv42.ExportAsync(Game, Combination, DrawCount, Direction);
        }
    }
}