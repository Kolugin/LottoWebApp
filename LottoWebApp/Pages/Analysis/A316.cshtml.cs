...
{
    [Authorize]
    public class A316Model : PageModel, IDisposable
    {
        private readonly AppServices _s;
        public A316Model(AppServices s)
        {
            _s = s;
        }
        ...
        public List<MinMaxDistributionResult> MinMaxDistributionResults { get; set; } = new();
        public int NumberOfBalls { get; set; }
        public int MaxBallNumber { get; set; }

        public class MinMaxDistributionResult
        {
        ...
        public void Dispose()
        {
            MinMaxDistributionResults.Clear();
        }

        public async Task OnGetAsync()
        {
            string cacheKey = $"A316_{Game}_{DrawCount}_{Direction}";
            if (string.IsNullOrEmpty(Game))
                Game = "keno";

            await GenerateMinMaxDistribution();
        }
        ...
        private async Task GenerateMinMaxDistribution()
        {
            ...
            var distribution = new Dictionary<(int Min, int Max), int>();
            int totalDraws = 0;
            ...
            foreach (var kvp in distribution.OrderBy(x => x.Key))
            {
                ...
                MinMaxDistributionResults.Add(new MinMaxDistributionResult
                {
                    Min = min,
                    Max = max,
                    Frequency = frequency,
        ...
        private (int Min, int Max) CalculateDrawStats(int[] balls)
        {
        ...
        public async Task<IActionResult> OnPostDownloadCsvAsync()
        {
            ...
            return await _s.Csv316.ExportAsync(Game, DrawCount, Direction, Title);
        }
    }
}