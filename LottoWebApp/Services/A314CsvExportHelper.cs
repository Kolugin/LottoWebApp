...
{
    public class A314CsvExportHelper
    {
        private readonly LottoDbContext _db;
        public A314CsvExportHelper(LottoDbContext db)
        {
            _db = db;
        }
        ...
        public async Task<FileResult> ExportAsync(string? game, int? drawCount, string? direction, string? title)
        {
            ...
            dt.Columns.Add("Среднее", typeof(double));
            dt.Columns.Add("Медиана", typeof(double));
            ...
            foreach (var kvp in distribution.OrderBy(x => x.Key))
            {
                ...
                dt.Rows.Add(average, median, frequency, probability);
            }

            string titlePart = string.IsNullOrEmpty(title) ? "A314_Average_Median_Distribution" : title;
