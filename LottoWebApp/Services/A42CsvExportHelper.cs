...
{
    public class A42CsvExportHelper
    {
        private readonly LottoDbContext _db;
        public A42CsvExportHelper(LottoDbContext db)
        {
            _db = db;
        }

        public async Task<FileResult> ExportAsync(string game, string combination, int? drawCount, string? direction)
        {
            ...
            csvContent.AppendLine($"Статистика для комбинации: {combination}");
            csvContent.AppendLine($"Комбинация выпала {occurrences} раз(а)");
            csvContent.AppendLine($"Последний раз выпала: {(lastOccurrenceDate ?? "нет данных")}");
            ...
            string fileName = $"4.2_{game.ToUpper()}_Статистика_комбинации_{combination.Replace(" ", "_")}_{DateTime.Now:yyyyMMddHHmmss}.csv";
