...
{
    public class A311CsvExportHelper
    {
        private readonly LottoDbContext _db;
        public A311CsvExportHelper(LottoDbContext db)
        {
            _db =db;
        }
        ...
        public async Task<FileResult> ExportAsync(string? game, int? drawCount, string? direction, string? title)
        {
            ...
            dt.Columns.Add("№ Колонки", typeof(int));
            dt.Columns.Add("Числа комбинации", typeof(string));
            ...
            foreach (var col in columnDistribution)
            {
                foreach (var pair in col.Value)
                {
                    ...
                    dt.Rows.Add(col.Key, pair.Key, pair.Value, chance);
                }
            }

            string titlePart = string.IsNullOrEmpty(title) ? "A311_Column_Distribution" : title;
