...
{
    public class A116CsvExportHelper
    {
        private readonly LottoDbContext _db;
        public A116CsvExportHelper(LottoDbContext db)
        {
            _db = db;
        }
        ...
        public async Task<FileResult> ExportAsync(string? game, int? drawCount, string? direction, string? title)
        {
            ...
            foreach (var year in yearNumberCounts.Keys.OrderBy(y => y))
            {
                ...
                foreach (var num in frequentNumbers)
                {
                    ...
                    dt.Rows.Add(year, num.Key.ToString(), num.Value.ToString(), chance.ToString());
                }
            }

            string titlePart = string.IsNullOrEmpty(title) ? "A116_Bonus_Ball_By_Year_Frequency" : title;
