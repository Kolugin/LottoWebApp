...
{
    public class A316CsvExportHelper
    {
        private readonly LottoDbContext _db;
        public A316CsvExportHelper(LottoDbContext db)
        {
            _db = db;
        }
        ...
        public async Task<FileResult> ExportAsync(string? game, int? drawCount, string? direction, string? title)
        {
            ...
            dt.Columns.Add("Минимум", typeof(int));
            dt.Columns.Add("Максимум", typeof(int));
            ...
            foreach (var kvp in distribution.OrderBy(x => x.Key))
            {
                ...
                dt.Rows.Add(min, max, frequency, probability);
            }

            string titlePart = string.IsNullOrEmpty(title) ? "A316_Min_Max_Distribution" : title;
