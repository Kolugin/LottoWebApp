...
{
    public class _536DownloadService : BackgroundService
    {
        private readonly IServiceProvider _serviceProvider;
        private readonly ILogger<_536DownloadService> _logger;

        // Определяем дни недели, в которые нужно запускать
        private static readonly HashSet<DayOfWeek> ScheduledDays = new HashSet<DayOfWeek> { DayOfWeek.Wednesday, DayOfWeek.Friday, DayOfWeek.Sunday };

        public _536DownloadService(IServiceProvider serviceProvider, ILogger<_536DownloadService> logger)
        {
        ...
        {
            _logger.LogInformation("Служба загрузки 5/36 запущена.");

            // Сделать первую проверку/загрузку при старте, если дата не сегодня
            ...
            while (!stoppingToken.IsCancellationRequested)
            {
                try
                {
                    // Вычисляем время следующего запуска (21:30 в ближайший Ср, Пт, Вс)
                    var nextRunTime = GetNextScheduledTime();
                    var now = DateTime.Now;
                    var delay = nextRunTime - now;

                    _logger.LogInformation($"Следующая загрузка 5/36 запланирована на {nextRunTime:yyyy-MM-dd HH:mm:ss} ({nextRunTime:dddd}). Ожидание {delay.TotalSeconds} секунд.");
                    ...
                    await downloader.Download536ResultsAsync(); // Проверяем и загружаем новый тираж

                    _logger.LogInformation($"Загрузка 5/36 выполнена в {DateTime.Now:yyyy-MM-dd HH:mm:ss}.");
                }
                catch (TaskCanceledException)
                {
                    // Task.Delay был прерван через cancellation token - выходим из цикла
                    _logger.LogInformation("Ожидание загрузки 5/36 прервано.");
                    break;
                }
                catch (Exception ex)
                {
                    _logger.LogError(ex, "Ошибка в службе загрузки 5/36.");
                    ...
                    catch (TaskCanceledException)
                    {
                        ...
                        _logger.LogInformation("Ожидание после ошибки загрузки 5/36 прервано.");
                        break;
                    }
                }
            }

            _logger.LogInformation("Служба загрузки 5/36 остановлена.");
        }

        /// <summary>
        /// Вычисляет ближайшее время запуска (21:30) в один из запланированных дней недели (Ср, Пт, Вс).
        /// </summary>
        ...
        {
            _logger.LogInformation("Проверка даты последнего тиража 5/36 при запуске службы.");
            ...
            if (!lastDrawDate.HasValue || lastDrawDate.Value < today)
            {
                ...
                var downloader = scope.ServiceProvider.GetRequiredService<LotteryDataDownloader>();
                await downloader.Download536ResultsAsync();
                _logger.LogInformation("Первоначальная загрузка 5/36 завершена.");
            }
            else
            {
