...
{
    public class A318CsvExportHelper
    {
        private readonly LottoDbContext _db;
        public A318CsvExportHelper(LottoDbContext db)
        {
            _db = db;
        }
        ...
        public async Task<FileResult> ExportAsync(string? game, int? drawCount, string? direction, string? title)
        {
            ...
            dt.Columns.Add("Низкие числа", typeof(int));
            dt.Columns.Add("Высокие числа", typeof(int));
            ...
            foreach (var kvp in distribution.OrderBy(x => x.Key))
            {
                ...
                dt.Rows.Add(low, high, frequency, chance);
            }

            string titlePart = string.IsNullOrEmpty(title) ? "A318_Low_High_Distribution" : title;
        ...
        private (int LowCount, int HighCount) CalculateDrawStats(int[] balls, int maxBallNumber)
        {
