...
{
    public class A114CsvExportHelper
    {
        private readonly LottoDbContext _db;
        public A114CsvExportHelper(LottoDbContext db)
        {
            _db = db;
        }
        ...
        public async Task<FileResult> ExportAsync(string? game, int? drawCount, string? direction, string? title)
        {
            ...
            foreach (var month in monthNumberCounts.Keys.OrderBy(m => DateTime.ParseExact(m, "MMMM", culture)))
            {
                ...
                foreach (var num in frequentNumbers)
                {
                    ...
                    dt.Rows.Add(month, num.Key.ToString(), num.Value.ToString(), chance.ToString());
                }
            }

            string titlePart = string.IsNullOrEmpty(title) ? "A114_Bonus_Ball_By_Month_Frequency" : title;
