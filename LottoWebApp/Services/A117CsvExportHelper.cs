...
{
    public class A117CsvExportHelper
    {
        private readonly LottoDbContext _db;
        public A117CsvExportHelper(LottoDbContext db)
        {
            _db = db;
        }
        ...
        public async Task<FileResult> ExportAsync(string? game, int? drawCount, string? direction, string? title)
        {
            ...
            foreach (var entry in dayMonthWeekNumberCounts)
            {
                ...
                foreach (var num in frequentNumbers)
                {
                    ...
                    dt.Rows.Add(dayOfMonth, dayOfWeek, num.Key.ToString(), num.Value.ToString(), chance.ToString());
                }
            }

            string titlePart = string.IsNullOrEmpty(title) ? "A117_Bonus_Ball_By_DayMonthWeek_Frequency" : title;
